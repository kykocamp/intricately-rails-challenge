    ...
    let(:params) do
      {
        dns_records: {
          ip: '5.5.5.5',
          hostnames_attributes: [{ hostname: 'dolor.com' }, { hostname: 'sit.com' }]
        }
      }
    end

    context 'when dns record is saved correctly' do
      let(:dns_record) { create(:dns_record, ip: '5.5.5.5') }
      let(:expected_response) { { id: dns_record.id } }

      before do
        allow(::DnsRecordCreateRepository).to receive(:create).and_return(dns_record)
      ...
      it 'returns the dns record created id' do
        expect(parsed_body).to eq expected_response
      end
    end

    context 'when dns record is not saved and raises an error' do
      before { allow(::DnsRecordCreateRepository).to receive(:create).and_raise(ActiveRecord::RecordInvalid) }

      it { expect { post(:create, params: params, format: :json) }.to raise_error(ActiveRecord::RecordInvalid) }
    end
  end
end
